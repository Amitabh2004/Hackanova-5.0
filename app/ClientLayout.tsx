    ...
    <>
      ...
      <aside className="font-boston fixed z-40 flex items-center bg-black/5 backdrop-blur-sm border border-white/5 shadow-2xl pointer-events-auto md:h-fit md:left-0 md:top-1/2 md:-translate-y-1/2 md:ml-4 md:flex-col md:rounded-2xl md:py-10 md:px-3 md:gap-5 bottom-0 left-0 w-full md:w-auto flex-row justify-around py-3 rounded-t-2xl md:rounded-2xl">
        {sidebarItems.map((item) => (
          ...
          >
            ...
            <span className="text-[8px] md:text-[10px] uppercase tracking-wider text-white/70 group-hover:text-white transition-colors">
              {item.name}
            </span>
          </Link>
        ))}
      ...
      <aside className="hidden md:flex fixed z-40 right-4 top-1/2 -translate-y-1/2 flex-col items-center gap-6 bg-white/5 backdrop-blur-md border border-white/10 py-8 px-3 rounded-full pointer-events-auto">
        {socialItems.map((social) => (
          ...
          >
            <span className="absolute right-full mr-4 px-2 py-1 bg-neon-cyan text-black text-[10px] uppercase tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
              {social.name}
            </span>
