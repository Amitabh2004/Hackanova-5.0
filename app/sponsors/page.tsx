...
export default function Sponsors() {
  ...
  // Prepended Devfolio to the grid for bot visibility
  const sponsorLogos = [CLD_ASSETS.DEVFOLIO_LOGO, ...CLD_ASSETS.SPONSOR_LOGOS];
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <div className="stars-container absolute inset-0">
          {STATIC_STARS.map((star, i) => (
            <div key={i} className="star absolute bg-white rounded-full opacity-0" style={{ top: `${star.top}%`, left: `${star.left}%`, width: `${star.size}px`, height: `${star.size}px`, boxShadow: star.glow, animation: `twinkle ${star.duration}s infinite ${star.delay}s` }} />
          ))}
        </div>
      </div>
            ...
            <div key={i} className="group relative aspect-video bg-white/5 border border-white/10 backdrop-blur-sm overflow-hidden transition-all duration-500 hover:border-neon-cyan/50 hover:bg-white/10 rounded-sm">
              ...
              <div className="relative w-full h-full transition-transform duration-700 group-hover:scale-110">
                <Image
                  ...
                  fill
                  unoptimized={i === 0} // Crawler compatibility
                  className="object-contain p-4 brightness-90 group-hover:brightness-110 transition-all"
